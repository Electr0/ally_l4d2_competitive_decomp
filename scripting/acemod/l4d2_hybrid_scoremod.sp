...
public OnReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	new revivee = GetClientOfUserId(GetEventInt(event, "subject"));
	new bool:ledgeHang = GetEventBool(event, "ledge_hang");
	if (ledgeHang)
	{
		CreateTimer(0.1, ReaddTempHealth, revivee, TIMER_FLAG_NO_MAPCHANGE);
	}
	return 0;
}
public Action:ReaddTempHealth(Handle:timer, any:client)
{
