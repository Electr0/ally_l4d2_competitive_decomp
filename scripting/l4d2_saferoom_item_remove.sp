...
public OnPluginStart()
{
	...
	g_hCvarSaferoom = CreateConVar(		"sm_safeitemkill_saferooms", 	"1", 	"Saferooms to empty. Flags: 1 = end saferoom, 2 = start saferoom (3 = kill items from both).", FCVAR_PLUGIN, true, 0.0, false, 0.0);
	g_hCvarItems = CreateConVar(		"sm_safeitemkill_items", 		"7", 	"Types to rmove. Flags: 1 = health items, 2 = guns, 4 = melees, 8 = all other usable items", FCVAR_PLUGIN, true, 0.0, false, 0.0);
...
RemoveEndSaferoomItems()
{
    ...
    for (new i=1; i < entityCount; i++)
    {
        ...
        if (checkItem == ITEM_KILLABLE || view_as<eTrieItemKillable>(GetConVarInt(g_hCvarItems)) & checkItem)
        {
